...
// Utilisation: node backup-restore-point.js pour restaurer

const fs = require('fs');
const path = require('path');

// Contenu de tous les fichiers critiques sauvegardés
const backupFiles = {
  '/home/z/my-project/src/app/page.tsx': `'use client'
...
export default function TravelPlanner() {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 404) {
          ...
          alert(errorData.error || 'Aucune activité trouvée pour ce thème')
        } else {
            ...
            <CardContent className="space-y-6">
              ...
              <div className="flex gap-3">
                <Button 
                  ...
                  className={\`flex-1 \${suggestedActivities.length > 0 ? 'bg-white text-gray-900 hover:bg-gray-50 border border-gray-300' : 'bg-green-600 hover:bg-green-700 text-white'}\`}
                  size="lg"
                >
                  {isLoading ? (
                    <>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <CardTitle className="flex items-center gap-2">
                      ...
                      Itinéraire pour {travelInfo.city}
                    </CardTitle>
                    <CardDescription>
                      {travelInfo.startDate?.toLocaleDateString('fr-FR', { day: '2-digit', month: '2-digit', year: 'numeric' })} au {travelInfo.endDate?.toLocaleDateString('fr-FR', { day: '2-digit', month: '2-digit', year: 'numeric' })} • {travelInfo.participants} personne(s) • {travelInfo.budget}
                    </CardDescription>
                  ...
                  {day.activities && day.activities.length > 0 ? (
                      ...
                      >
                        <div className="flex-shrink-0">
                          {activity.image ? (
                            ...
                            <div key={\`placeholder-\${activity.id}\`} className="w-12 h-12 bg-gray-200 rounded-lg flex items-center justify-center">
                              <ImageIcon className="w-6 h-6 text-gray-400" />
                            </div>
                          )}
                        </div>
                    ...
                    >
                      <div className="flex flex-col items-center gap-2">
                        ...
                        <p className="text-sm">Glissez une activité ici ou utilisez le menu déroulant</p>
                      </div>
                    </div>
                  )}
                </div>
                
                {/* Menu déroulant pour ajouter des activités */}
                <div className="mt-4">
                  <Select onValueChange={(value) => {
                    ...
                    <SelectTrigger className="w-full">
                      <SelectValue placeholder="Ajouter une activité à ce jour..." />
                    </SelectTrigger>
                    <SelectContent>
                      {selectedActivities
                        .filter(activity => 
                          !day.activities || !day.activities.some(a => a.id === activity.id)
                        )
                        .map((activity) => (
  ...
  '/home/z/my-project/src/components/ui/header.tsx': `"use client"
...
export function Header() {
  ...
  '/home/z/my-project/src/components/ui/travel-form.tsx': `"use client"
...
export function TravelForm({ value, onChange }: TravelFormProps) {
  ...
  '/home/z/my-project/src/app/api/more-activities/route.ts': `import { NextRequest, NextResponse } from 'next/server'
...
export async function POST(request: NextRequest) {
  try {
    ...
    // NE PLUS AJOUTER d'autres activités si le thème n'en a pas assez
    // On retourne seulement les activités du thème demandé
    ...
    if (activities.length === 0) {
      ...
      return NextResponse.json(
        { error: \`Aucune activité trouvée pour le thème "\${theme}" à \${city}\` },
        { status: 404 }
      )
    }
...
function restoreFiles() {
  ...
  console.log(\`✅ Fichiers restaurés: \${successCount}\`);
  if (errorCount > 0) {
    console.log(\`❌ Erreurs: \${errorCount}\`);
  }
  console.log(\`\\n📝 Date de la sauvegarde: 2025-06-17\`);
  console.log(\`🎯 Version: GoVisitCity Travel Planner - État stable\`);
}

// Exécuter la restauration si ce fichier est lancé directement
if (require.main === module) {
  restoreFiles();
}

module.exports = { restoreFiles, backupFiles };`